...
logErr = function (message, err) {
    ...
    if(thisAdapter.initilised){
        ...
        if (swarmToSwarmLevel <= 2) {
            if (message != undefined && message.indexOf("Error while processing Redis commands") == -1 && thisAdapter.mainGroup != 'Logger') {
                console.log("Zaxa: %s, swarm stuff: %j", thisAdapter.mainGroup, getCurrentSwarm());
